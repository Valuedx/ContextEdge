...
Every similarity query against these two surfaces — evidence and chunks — goes through `halfvec_cosine_distance`, matching the `0032` halfvec expression indexes (verified: the only raw `.cosine_distance(...)` call sites left in the backend are two episode-embedding filters, backend/src/contextedge/workers/pattern_tasks.py:211 and :266). See the HNSW design decision below.
