...
3. **Embeddings** — `generate_embedding` calls `litellm.aembedding` with fixed **3072** dimensions (aligned with Vertex / model expectations). Used by `embed_evidence` / `embed_decision` in `ai/embeddings.py` (both now thread `tenant_id` + `db` kwargs through to `generate_embedding` so embedding spend lands in `/admin/cost` and respects the pre-call budget gate — review C-04/F-03) and by search (`vector_search`, `hybrid_ranker` query embedding). `embed_evidence_batch` fills sparse texts with zero vectors to keep array shapes consistent.
   ...
   Two embedding surfaces now coexist on evidence:

   - **Parent embedding** — `EvidenceItem.embedding`, generated inline in `_normalize` from `embed_evidence(title, body)`. Drives contradiction scanning, similar-decision retrieval, and baseline matching. Unchanged by the chunking work.
   - **Chunk embeddings** — `EvidenceChunk.embedding`, generated by `embed_chunks_batch_task` (`workers/chunk_tasks.py`) in batches of `EMBED_BATCH_SIZE = 32` via `generate_embeddings_batch`. Land asynchronously after `write_chunks` writes the rows; the chunk row exists with `embedding = NULL` for a brief window until the batch task fires. Per-tenant LLM budget enforcement is per-batch — same gate as parent embeddings.

   Splitting the chunk embed off from the chunk write keeps `_normalize`'s critical path bounded for big attachments. Detail in [`CHUNKING_DESIGN.md`](./CHUNKING_DESIGN.md) §3 ("Where it slots into `_normalize`") and §"Design decisions: embedding split from chunking."
...
- **HNSW indexes on embedding columns** (since 2026-04-22, migration `0021_hnsw_vector_indexes`) — *Why:* unindexed cosine-distance queries scan the full 3072-dim column linearly; at several million rows this dominates runtime similarity queries. HNSW gives ~95% recall at roughly 100× the throughput. *Tradeoff:* approximate rather than exact neighbours; index maintenance cost on inserts (bounded — pgvector's HNSW implementation is designed for this). Runtime recall is tunable per session via `SET LOCAL hnsw.ef_search = <n>`. Migration `0030_evidence_chunks` extends this pattern to `evidence_chunks.embedding` with the same `m = 16`, `ef_construction = 64` parameters.

- **Chunk embeddings batched, not inline** (since 2026-05-08) — *Why:* `_normalize` is on the critical ingest path. Embedding 50 chunks of a long item inline would push p95 latency past acceptable bounds and would re-fire the per-tenant LLM budget gate per chunk. Batching via `generate_embeddings_batch(texts)` lets one LLM call cover up to 32 chunks; the budget gate runs once per batch. *Tradeoff:* there's a brief window after `write_chunks` lands where chunks exist but `embedding IS NULL` — the chunk-vector query handles this naturally (the `<=> :q ORDER BY` sees no rows), and `embed_chunks_batch_task` is idempotent on retry via the same `IS NULL` filter.
...
| Chunk embeddings | `backend/src/contextedge/workers/chunk_tasks.py` | `embed_chunks_batch_task`, `EMBED_BATCH_SIZE` | Celery **extraction** queue |
