...
- **Sync runs as first-class rows** — *Why:* operators see history, failures, counts, and the handoff-recovery blob in the UI/API; retry is a button, not a shell session. *Tradeoff:* run-log retention needs housekeeping — hence the delete/purge endpoints (api/v1/sync.py:64-87).
