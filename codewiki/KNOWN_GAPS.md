...
- **The `0029` execution/decision governance columns have no writers [Gap].** `ExecutionStepRun.idempotency_key` / `duplicate_check_status` / `action_name` / `action_type` / `execution_mode` / `executed_by`, `ApprovalRequest.recommended_by` / `sod_check_status` / `sod_violation_reason` / `action_name` / `approver_role` / `approval_channel`, and `Decision.decision_intent` / `policy_result` / `risk_level` are written by nothing — the only non-model reference is a *reader* in `graph/agent/hydrators.py`. Consequences: `uq_execution_step_runs_idempotency_key` guards a column that is always NULL, so the "banking-grade" duplicate control is inert; SoD is enforced only on the initiator↔approver axis via `forbid_self_approval`, never recommender↔approver. This was deliberate (`codewiki/17` lists service-code population as out of scope for `0029`) but now reads as shipped capability to anyone auditing the schema. **F1.**
- **Correction to the entry below:** the 2026-08-05 line "Outcome loop is schema-only (P0-4)" is **stale** — `services/case_outcome_service.py` now writes `CaseOutcome`, `CaseStateTransition` and `CaseOutcomeFixPattern`. That gap is closed; the `0029` execution/decision columns above are the remaining instance of the same pattern.
