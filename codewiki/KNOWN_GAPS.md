...
- **Knowledge validation is not wired into retrieval [Gap].** `knowledge_validation_service` computes empirical support levels from *verified* executions; `knowledge_retrieval_service` ranks on distance × `rank_penalty` only, so a superseded SOP still ranks on similarity alone. Document "versioning" (`services/documents/versioning.py`) is a filename/hash heuristic producing in-memory dataclasses — no `KnowledgeVersion` table, no persisted supersession (`superseded_by` edges are written only for claims). **F4.**
