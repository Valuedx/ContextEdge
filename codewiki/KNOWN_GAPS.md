...
- **Reply-inheritance ordering** — a Teams reply that correlates before its parent gains any case membership never retries inheritance (pending-mention reconciliation covers ticket tokens, not reply chains). Thread-level correlation edges still cluster the messages, so episode grouping is unaffected — the cost is membership precision. Fix direction: reconcile reply chains when a parent membership lands, or re-run `inherit_reply_membership` during debounced reconstruction.
- **Dissociation veto is a phrase list; bot replies inherit at full tier confidence** — the veto in `ticket_bridge_service.DISSOCIATION_PHRASES` is deliberately conservative v1; the message-function classifier (explicit_dissociation, bot-authored weighting) is the planned replacement. A digest-downgraded `mentioned_only` row also blocks a later `reply_inheritance` upgrade for the same (evidence, case) pair — first-writer-wins keeps it conservative.
- **Suggestion queue volume** — semantic correlation suggestions (migration 0039) have per-run caps (5 suggestions, 6 query chunks) but no per-tenant queue cap: a large backfill can accumulate a big pending-review backlog. The gates (similarity floor, corroborator requirement, permanent rejection) bound noise per pair, not total volume. Reviewer console for the queue is API-led for now (list/accept/reject under `/correlations/suggestions`).
- **Wrong-source-attribution rate is not measured** — P4 ships the day-1 proxy (per-episode `episode.synthesis_quality` log with the unsupported-step rate and contradiction count), but measuring whether a step cites the *right* source needs labeled evaluation data (an eval dataset with per-step gold citations). Belongs with the evaluation harness work.
- **Episode contradictions are review-surface only** — `episodes.contradictions` (0040) reaches the API/reviewers; the MAF agent projection does not render them yet, so an agent consuming an episode does not see that its sources disagreed.
