...
- **No scoped trust model [Gap].** Autonomy is a global mode on the playbook; there is no (agent × action × resource class × environment × criticality) track record. The raw material exists — cohort counters, calibration buckets, verification verdicts. **F10.**
