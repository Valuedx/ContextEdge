...
- **Model/prompt provenance never reaches the derived entity [Gap].** `prompt_version` and model flow into `llm.usage` operational events, but the payload carries no episode/decision/claim id, and the event is skipped entirely when no `db` session is in scope. **F5.**
