...
- **`action_policies` is not in the enforcement path [Gap].** Read only by the agent projection (`graph/agent/materializer.py`, `hydrators.py`); `execution_service` never queries it, and there is no CRUD API, so rows arrive via seed scripts. Gating comes from `TenantPolicy` through `approval_policy_service` (which *is* enforced and fails closed). Policies are also unversioned — no `version` / `effective_from` / `effective_to`, and `DecisionActionPolicy.policy_result_snapshot` records a result with no policy version, so "which policy version evaluated this?" is unanswerable. **F3.**
