...
- **Approval is not bound to an artifact [Gap].** No content hash on `PlaybookVersion`, no `artifact_version` / `artifact_hash` / policy snapshot / `expires_at` on an approval, and no pre-execution re-check. `PlaybookVersion.steps` is mutable JSONB, so "which exact artifact did the human approve" cannot be expressed. Related: `approval_expiry_service` writes `status = "expired"`, which is absent from `APPROVAL_STATUSES` and has no CHECK behind it. (The 72h *pending-request* expiry shipped as the E6 safety slice and is unaffected.) **F7.**
