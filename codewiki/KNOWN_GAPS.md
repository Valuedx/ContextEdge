...
- **`edge_type` is unvalidated free text [Gap].** Written from the 26 modules that import `graph.builder`; the maf.v1 allowlist governs reads only, so a typo at a write site is invisible until the edge silently fails to project. **F2.**
