...
- `contradiction_service.scan_contradictions` uses a **three-gate** flow to keep LLM spend bounded:
  1. **HNSW top-K KB candidates** — for each `(playbook_version, step)` pair, the scanner pulls only the `top_k` (default 20) most similar KB-typed evidence rows via `pgvector` HNSW cosine search, instead of the old full-KB sweep. All predicates — tenant, evidence_type, embedding-not-null, domain scope, and the `exclude_legal_hold()` filter — are applied before `ORDER BY cosine_distance` so the HNSW index actually cooperates with the filter rather than being defeated by post-LIMIT filtering (review F-22). Legal-hold evidence never reaches the LLM (review F-23).
  ...
  3. **Token-overlap gate + LLM budget** — surviving pairs still go through the token-overlap heuristic as the final pre-LLM filter. Gate tightened to `overlap == 1 AND min(len) <= 3` (review F-25) — single-word matches only slip through against genuinely short fragments. A hard `max_llm_calls` cap (default 1000) terminates the scan when exhausted; all three loop levels break out cleanly instead of spinning through the rest of the candidate set (review F-26). The return dict reports `llm_calls_used`, `token_skips`, `cursor_skips`, `budget_skips`, `budget_exhausted` for observability.
- `_get_or_create_contradiction` catches `IntegrityError` against the `uq_contradictions_tenant_source_pair` unique index (migration `0026`) so concurrent scanners can't silently insert duplicates (review F-24). On conflict it rolls back, re-SELECTs the winning row, and merges mutable fields.
- The system/user prompt is registered at `ai/prompts/contradiction.py` (versioned, A/B-routable per tenant). On a hit, an LLM call persists a `Contradiction` row, adds graph edges, emits an operational event, and can notify via `notification_service`. Notifications are capped at 10 individual sends per scan and a single summary notification fires at scan end if the cap was hit (review F-27).
