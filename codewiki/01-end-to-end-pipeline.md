...
12. **Patterns and playbook candidates** — `pattern.cluster_episodes` (**pattern** queue, `backend/src/contextedge/workers/pattern_tasks.py:379`) is **event-driven, not scheduled**: episode approvals and the AI-review sweep dispatch it per domain (plus a manual API). It matches approved, embedded episodes against existing patterns (embedding distance < 0.35 plus an LLM adjudication) or clusters semantic neighbors (distance < 0.20) and synthesizes a new pattern. New or grown patterns auto-enqueue `pattern.generate_playbook_candidate` (`backend/src/contextedge/workers/pattern_tasks.py:403`), which refuses patterns below a 0.5 confidence floor (`backend/src/contextedge/workers/pattern_tasks.py:34`), retrieves relevant KB/SOP knowledge to ground the draft, drops any citation the model invented, and persists a **candidate** playbook with full evidence provenance. Details in [07](./07-episodes-patterns-playbooks.md).
