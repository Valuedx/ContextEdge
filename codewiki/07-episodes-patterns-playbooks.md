...
- **Resolution gate** (`EPISODE_RESOLUTION_GATE`, default `off`; `services/resolution_signal_service.py`): with `cluster` set, scheduled reconstruction **defers** for clusters carrying no resolution signal anywhere — no status vocabulary, no resolution phrasing, nothing in the v2 relevance summaries (which are scanned as already-paid classifier output). Deferred, not dropped: evidence, embeddings, and case links persist, and the next correlate→reconstruct dispatch re-checks once new evidence joins — essential for scattered-source deployments where the problem arrives from one system and the fix from another days later. The check is deterministic (regex over title/summary/body head, no LLM call), a **cluster** property rather than an evidence filter (dropping problem-side evidence would break correlation joins), fails open on errors, and never argues with a manual reviewer trigger (`settle=False` bypasses). Ambiguous phrases err permissive by design: a false positive merely spends what today's ungated behavior spends, while gating single-source deployments at the ticket level belongs to connector status filters, not this knob.
...
- **Dedup on a clock** (2026-08-18): the containment + similarity passes were wired into the shared entry point but nothing scheduled it, so drafts accumulated between manual runs — measured at 643 → 2,869 pending across one bulk-ingest night, consolidated to ~950 by a single sweep. `pattern.deduplicate_knowledge` now runs hourly from beat. *Why hourly:* the passes are idempotent and near-free when there is nothing to do, and the reviewer surface should be self-cleaning rather than operator-dependent. *Tradeoff / guard:* sweeping mid-backfill retires drafts the next message burst regrows, so the task defers per-tenant while recent ingest volume is above `DEDUP_ACTIVITY_THRESHOLD` (50 evidence rows / 10 min — above steady-state, below backfill rates); a busy tenant is skipped, not queued, and the next hourly tick catches up. Routed `pattern.*` so the solo pattern worker serializes it behind clustering and playbook generation instead of racing them.
- **AI first-pass episode review** (2026-08-18, `EPISODE_AI_REVIEW`): an hourly sweep (plus on-demand dispatch from the episodes page) reads each pending draft against its own evidence and stamps a verdict on `episodes.ai_review`; in `auto_approve` mode it also approves drafts clearing BOTH the model verdict AND deterministic floors (≥2 evidence, substantive outcome, verdict confidence ≥0.8 — `episode_review_service`). *Why:* human review is the pipeline's long pole (1,400 drafts after one bulk ingest), and a first-pass filter that annotates everything and approves only the unambiguous subset moves real workload without moving authority — the model proposes, policy disposes, and a dispatch argument can only downgrade the configured mode, never escalate it. Auto-approvals keep `reviewer_user_id` NULL so they are permanently distinguishable. *Tradeoff (measured 2026-08-18, 20+20 calibration):* superseded fragments 0/20 approved — the dangerous direction is clean; human-approved episodes only 4/20 would auto-approve, so the yield is deliberately conservative (~20% on a weak ground-truth set that includes early bulk approvals) and the feature reduces review load rather than replacing review. ~$0.004/review. Ships default `off`; recommended rollout is `advisory` first, then `auto_approve` once the annotations earn trust.
