...
In the Acme VPN outage, the dashboard is the business user's path through the product: an admin sees the source health on Overview, a responder opens Evidence and Sessions to understand the outage, a knowledge manager updates Negative Knowledge and Playbooks after learning what failed, and an approver uses Execution to allow or deny a higher-risk recovery action. The Graph Explorer now shows the full decision trail — from the session that triggered execution, through the approval chain, to the action that resolved the issue — alongside AI-extracted decisions from evidence text (e.g., "engineer restarted vpn-gw-east-01"). The same incident appears as one governed workflow rather than a disconnected set of backend APIs.
