...
### 5b. Graph Explorer — read-only traversal of the context graph

Three tabs over `GET /graph/*`: **Statistics** (total edges, node-type distribution, edge-type breakdown), **Subgraph** (any entity type/ID and depth, rendered with React Flow and dagre auto-layout, clicking a node re-centres the view on it), and **Neighbors** (breadth-first traversal via `GET /graph/neighbors`, grouped by depth, with edge-type filtering and follow-node navigation). All three accept an optional `domain_id` scope. Shared styling lives in `graph-constants.ts` and is reused by the pattern-scoped graph view.

The vocabularies cover the decision trail — `session`, `execution_run`, `approval_request`, `user` nodes (`frontend/src/components/graph/graph-constants.ts:11-13, 62-64`) and edges such as `records_decision` (`graph-constants.ts:93`) — plus the AE-alignment node types `claim`, `action_policy`, `error_signature`, `fix_pattern`, and `case_outcome` (`graph-constants.ts:1-24`), so an operator can trace who decided what and which failure fingerprint recurred. Two honest caveats: `issue_signature`, the LLM-derived fingerprint minted on episode approval, is in neither `NODE_TYPE_OPTIONS` nor `nodeColors`, so it falls back to the neutral slate style in `getNodeClassName` (`graph-constants.ts:125-129`); and `fix_pattern` is offered as a filter for a table nothing writes yet (see [17-ae-ops-context-graph-alignment.md](./17-ae-ops-context-graph-alignment.md)). The page is read-only — every graph mutation happens in backend services, from pattern discovery, playbook generation, contradiction scans, identity linking, decision extraction, and episode graph construction.
...
5. **Alerts** for a stalled chain, for a cold-start tenant whose identities are all `provisional`, for more than 500 tasks on the extraction lane, and — critical — for zero evidence produced in 10 minutes while work is still queued (`pipeline_health_service.py:223-274`).
...
Also `tenant_admin` on both sides (`sidebar-nav.tsx:67`; `admin_cost.py:33-60`). Headline KPI cards over a selectable window — estimated USD, total tokens, cache-hit rate, average cost per request — plus a top-N model × task breakdown with a CSS-only stacked bar splitting prompt-non-cached / cached / completion per row (`frontend/src/app/(dashboard)/admin/cost/page.tsx:102, 135, 565-580`). Cache-hit rate is tone-coded green ≥ 50%, amber ≥ 20%, red otherwise (`admin/cost/page.tsx:77`), so a rollout that should have lit up prompt caching and didn't is obvious at a glance.
...
| Add-source flow | `frontend/src/components/sources/add-source-dialog.tsx:165-167, 476` | `AddSourceDialog`, `handleBrowseFolder` | Source creation |
| Source-type catalog (picker truth) | `backend/src/contextedge/connectors/registry.py:37-88, 100-110` | `source_type_catalog`, `SourceTypeInfo` | `GET /sources/types` |
...
| Runtime sandbox | `frontend/src/app/(dashboard)/runtime/page.tsx:61, 197-249` | `RuntimePage`, `FeedbackTab` | Live retrieval and feedback |
| Sessions | `frontend/src/app/(dashboard)/sessions/page.tsx` | `SessionsPage`, `NewSessionDialog`, `SessionDetail` | Incident coordination |
| Reviewer console | `frontend/src/app/(dashboard)/review/page.tsx:61-99, 399-780` | `ReviewPage`, `QueuePane`, `RankedHypotheses`, `DecisionBar`, `RejectDialog`, `ModifyDialog`, `findPendingApproval` | Confidence-ranked pending decisions |
...
| Pipeline Health service | `backend/src/contextedge/services/pipeline_health_service.py:43-55, 58-84, 87-274` | `QUEUES`, `_queue_depths`, `get_pipeline_health` | `GET /admin/pipeline-health` |
| Admin cost + budget page | `frontend/src/app/(dashboard)/admin/cost/page.tsx:77, 102-192, 399, 565` | `AdminCostPage`, `KpiCard`, `BreakdownBar`, `BudgetPanel`, `BudgetEditForm` | `tenant_admin` |
| Budget evaluation | `backend/src/contextedge/services/tenant_budget_service.py:107-121, 191-282` | `check_budget`, `_DefaultBudget` | Before every LLM call |
| Admin cost routes | `backend/src/contextedge/api/v1/admin_cost.py:33-175` | `llm_usage`, `tenant_budget`, `pipeline_health` | Admin pages |
...
| Graph subgraph / neighbors / stats | `frontend/src/components/graph/graph-subgraph.tsx`, `graph-neighbors.tsx`, `graph-stats.tsx` | `GraphSubgraph`, `GraphNeighbors`, `GraphStats` | The three Graph Explorer tabs |
| Shared graph styling | `frontend/src/components/graph/graph-constants.ts` | `nodeColors`, `edgeColors`, `NODE_TYPE_OPTIONS`, `getNodeClassName` | All graph views |
