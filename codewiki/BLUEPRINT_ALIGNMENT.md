...
| 3 Service dependency map | yes + inferred | authored depends_on/runs_on/... projectable; **co_fails_with** inferred from case co-occurrence (dependency_inference_service); **proposed_depends_on** from agent discoveries (review-gated) |
...
| 5 Telemetry index | derived subset | **monitoring_sources** entity fact stamped from alert-shaped evidence (index_monitoring_sources); full monitor-config map needs Datadog/Splunk config-API connectors |
