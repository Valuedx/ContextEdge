...
**One limit of that guard, worth knowing before you trust it:** the scan matches on **column name**, not on model. When two tables share a column name and one of them gains a writer, the other becomes invisible to the scan. The register keeps six such columns as prose in `SHADOWED_BY_NAME` (`tests/test_governance_column_writers.py:222-236`), and this article names three more found by reading the constructors directly — `ApprovalRequest.case_id`, `ExecutionStepRun.case_id`, and `DecisionTraceEvent.tool_name`. A name collision costs visibility, not truth.
...
| `claims` | **Live** — created, never validated | `claim_service.persist_extracted_claims` (`services/claim_service.py:48, 77`), called from the relevance pass (`workers/extraction_tasks.py:452-455, 657-659`) |
...
| `action_policies` | **Live and enforcing** | Authored via `api/v1/action_policies.py:144`; evaluated per step at `start_execution` (`services/action_policy_service.py:156`; `services/execution_service.py:234-286`) |
| `error_signatures` | **Live** — key, message, examples only | `error_signature_service.fingerprint_evidence` (`services/error_signature_service.py:176, 216`), called on every normalize (`workers/extraction_tasks.py:511-514`) |
| `fix_patterns` | **Dormant** — nothing constructs a row | Nothing. Read in five places and written in none; see "The `FixPattern` hole" below |
| `case_outcomes`, `case_state_transitions` | **Live** | `case_outcome_service.record_case_transition` / `record_case_outcome` (`services/case_outcome_service.py:101, 126, 113, 161`), driven from session close (`services/session_service.py:198-238`) |
...
| `resolution_sessions` case-spine columns | **Dormant** (`case_number`, `case_type`, `issue_type`, `severity`, and the user / request / agent entity FKs) | Nothing; owner F3 (`tests/test_governance_column_writers.py:149-161`) |
...
| `graph_edges.valid_from` / `valid_to` / `confidence` | **Live** | `graph/builder.py:43, 108, 166` writes validity on every edge; `cmdb_topology_service.py:235` end-dates edges when upstream deletes a relationship; `ensure_edge` takes `confidence` separately from `weight` |
...
Polymorphic entity keyed on `(tenant_id, entity_type, external_system, external_id)` UNIQUE (`models/entity.py:67-75`, tightened from `0029`'s global key by `0031`). Designed to hold AE workflows, agent machines, schedules, and output locations.
...
`ENTITY_TYPES` is the registry (`models/entity.py:32-64`), and `tests/test_entity_type_registry.py` scans every reference service and fails the moment one writes a type the registry does not know. That test exists because Zoho enrichment was writing `customer_account`, `knowledge_category`, and `topic` unregistered — exactly the ontology drift the tuple is for.
...
| `claim_evidence` | Claim ↔ `evidence_items` link with `support_type` (`supports` / `contradicts` / `weakens`) and `weight`. `UNIQUE (claim_id, evidence_id)`. | **No constructor.** The link is written as a `claim -[supported_by]-> evidence` **graph edge** instead (`claim_service.py:91-100`). The relational table stays empty, so `support_type` has no writer. |
...
The partial index on `claims (tenant_id, created_at DESC) WHERE validation_status IN ('unverified', 'machine_verified')` (`0029_ae_ops_concept_alignment.py:134`) powers the "claims awaiting validation" dashboard tile — a tile that would have plenty to show and no way to act on it.
...
Enforcement runs per step inside `start_execution` (`services/execution_service.py:226-286`). Every evaluation writes a `policy_checks` row — `pass`, `fail`, or `not_applicable` when no policy matches — with the action name, step index, verdict, environment, and workflow scope in `input_snapshot` (`execution_service.py:241-279`). A blocking verdict raises `ExecutionPolicyError` naming the policy and its version (`:280-285`). The run's strictest verdict lands on `Decision.policy_result` (`execution_service.py:1005`).
...
- Every status change appends a transition row (`record_case_transition`, `:101-124`), called from session creation and status updates (`services/session_service.py:62`).
...
Read surface: `GET /api/v1/sessions/{id}/…` composes the history via `get_case_history` — transitions oldest-first because it reads as a timeline, outcomes newest-first because the latest is the operative one and earlier rows are reopen-and-close history (`services/case_outcome_service.py:38-44`; `api/v1/sessions.py:110-117`).
...
| Column | Why | Status |
|--------|-----|--------|
| `case_number` | Partial-unique, now tenant-scoped: `(tenant_id, case_number) WHERE case_number IS NOT NULL` (`0031:96-101`); generator shaped like `CG-YYYY-NNNNNN` | Unwritten — no generator exists |
| `case_type` / `issue_type` | Controlled vocab: `output_not_received`, `workflow_failed`, `agent_not_reachable`, `workflow_stuck`, `credential_failure`, `input_file_issue`, `db_connectivity_issue`, `schedule_not_triggered` | Unwritten |
| `title` / `description` / `priority` / `environment` | First-class columns instead of fishing them out of `entities[]` JSONB | Written |
...
| `redaction_status` | Per-row marker (`not_required` / `pending` / `applied`) | **Read but unwritten.** The agent projection hides evidence whose status is `pending` (`graph/agent/hydrators.py:163-164`), and vector search and episode clustering apply the same fence in SQL (`search/vector_search.py:59-60`; `services/episode_cluster_service.py:87-88`) — but redaction runs without stamping the row, so those gates can never fire |
...
| Column | Why | Status |
|--------|-----|--------|
| `decision_intent` | Design vocab, queryable alongside `decision_type` (`models/decision.py:137-142`) | Written (F1) |
...
| `risk_level` | **Trace-level** risk, distinct from per-option risk on `DecisionOption` (`models/decision.py:146` vs `:193`): "options considered had what risk" vs "the path we took has what risk" | Written (F1) |
...
| Column | Why | Status |
|--------|-----|--------|
| `valid_from` / `valid_to` | Enables "what was true at incident time?" (Section 43.1.4). Both nullable: existing edges keep behaving as "valid since creation, no expiry" (`models/pattern.py:184, 221-222`) | Written — `graph/builder.py:43, 108` on create, `close_edge` at `:166`, and `cmdb_topology_service.py:235` end-dates upstream-deleted relationships on TTL refresh |
| `confidence` | Semantically distinct from `weight` (importance for traversal): confidence is the belief in the relation, range-checked `0..1` (`models/pattern.py:180-181`) | Written — `ensure_edge` takes it as a separate argument, and the weight-as-confidence conflation it was added to fix was found in code written days earlier in this same repo |

A composite index on `(valid_from, valid_to)` supports the temporal lookup pattern. One honesty note carried from the graph review: `as_of` semantics are not fully coherent yet — historical edges combine with *current* node facts, so a caller must not draw historical operational conclusions from a point-in-time traversal.
...
| Action policy precedence engine (`priority`, `policy_scope`, `conflict_resolution`) | **Landed as F3b.** Scope filter → specificity → conflict resolution, enforced per step (`services/action_policy_service.py`; `services/execution_service.py:226-286`); versioned by `0064` |
...
When Acme's `INC0010427` — VPN users unable to connect to `vpn-gw-east-01` — flows through the pipeline, four `0029` tables get touched and two stay empty. That contrast is the fastest way to see where the alignment stands.
...
```json
{
  ...
  "error_signatures": [
    {"signature_key": "LOG_ERROR_IKE_AUTH_AUTH_CERT_EXPIRED_CERT_SERIAL",
     "error_type": "LOG_ERROR",
     "normalized_message": "<DATE> <TIME> ERROR ike_auth: AUTH_CERT_EXPIRED cert serial <HEX> for <HOST>",
  ...
  "graph_edges": [
    {"source": "evidence:b1f0…", "edge_type": "exhibits", "target": "error_signature:LOG_ERROR_IKE_AUTH_…"},
...
```json
{
  "case_state_transitions": [
    {"from_status": "executing", "to_status": "closed", "transitioned_by": "eng-42"}
  ],
  "case_outcomes": [
...
Read the two empty arrays. `claim_evidence` is empty because the claim↔evidence relationship is carried by a graph edge, and the relational link table has no constructor. `case_outcome_fix_patterns` is empty because `successful_action` names an action that no `FixPattern` row represents — so the outcome is recorded, the learning loop is not closed, and the counters in `fix_cohort_service` have nothing to count. Everything needed to close that loop exists except the thing that mints the row.

Meanwhile the `resolution_sessions` case spine — `case_number`, `case_type`, `issue_type`, and the four entity FKs that would let you ask "every case on `vpn-gw-east-01` this quarter" as a structured query rather than a graph traversal — stays NULL for this incident, because nothing writes it.

## Key Files

| File | Purpose |
|------|---------|
| `backend/alembic/versions/0029_ae_ops_concept_alignment.py` | The migration (tables at `:88`, `:134` for the claims partial index) |
...
| `backend/src/contextedge/models/entity.py:32-64, 65-99` | `ENTITY_TYPES` registry, `Entity`, tenant-scoped unique key, `0043` traits |
| `backend/src/contextedge/models/claim.py:57, 111, 145, 183` | `Claim`, `ClaimEvidence`, `DecisionEvidence`, `DecisionClaim`, lifecycle constants |
| `backend/src/contextedge/models/action_policy.py:54` | `ActionPolicy`, `RISK_LEVELS`, `POLICY_RESULTS`, `EXECUTION_MODES` |
| `backend/src/contextedge/models/error_signature.py:39, 99` | `ErrorSignature` (unique per tenant at `:41`), `FixPattern` |
| `backend/src/contextedge/models/case_outcome.py:60, 110, 136` | `CaseOutcome`, `CaseStateTransition`, `CaseOutcomeFixPattern`, `OUTCOME_STATUSES`, `CASE_STATUSES` |
| `backend/src/contextedge/models/session.py` | Case-spine columns + four entity FKs on `ResolutionSession`; decision anchors on `DecisionTraceEvent` |
...
| `backend/src/contextedge/models/decision.py:137-149` | `decision_intent` / `decision_summary` / `risk_level` / `policy_result` |
| `backend/src/contextedge/models/execution.py:112-124, 203-216` | Step action name + idempotency; approval channel/role/SoD anchors |
| `backend/src/contextedge/models/pattern.py:174-222` | `GraphEdge` validity window and confidence range check |
| **Writers** | |
| `backend/src/contextedge/services/claim_service.py:48-100` | `persist_extracted_claims` — claims + `supported_by` edges, no extra LLM call |
...
| `backend/src/contextedge/services/execution_service.py:226-286, 403-493, 1005` | Action-policy enforcement, idempotency-key assignment, strictest verdict onto the decision |
