...
`extraction.normalize_evidence` (backend/src/contextedge/workers/extraction_tasks.py:1319) wraps `_normalize` (extraction_tasks.py:122). The whole body is one transaction; every `.delay()` fan-out happens in the task wrapper **after** `run_async` commits (extraction_tasks.py:1325-1367).
...
- **The relevance gate short-circuits the expensive half.** `skip_extraction = (label == "not_relevant" and confidence >= 0.75)` (extraction_tasks.py:488-492). A skipped item keeps its row for audit but gets no message-function call, no identity resolution, no decision extraction, no parent embedding, and no chunking — it is invisible to vector search by construction. Classifier *failure* is fail-open: the `except` logs and falls through, and the item continues down the full path (extraction_tasks.py:475-482).
- **Error-signature fingerprinting is deterministic and runs on every item, including skipped ones** — a confidently-irrelevant thread can still carry a pasted stack trace (extraction_tasks.py:520-539).
- After the relevance gate, the worker runs **identity linking** (`link_evidence_identities`, extraction_tasks.py:546) and **decision extraction** (`link_evidence_decisions`, extraction_tasks.py:564) inline. Decision extraction uses an LLM to identify operational actions, resolves actors and targets through the same identity resolver, and creates `records_decision` / `records_action_on` graph edges.
- **Parent embedding** is ensured inline so semantic search sees newly normalized evidence without a second broker hop (extraction_tasks.py:65-70, 580-584).
...
After `_normalize` commits, the task wrapper dispatches (extraction_tasks.py:1327-1364):
...
2. `hydration.hydrate_thread`, but only when the payload carried a `_thread_id`, the record is **not itself a hydrated message**, and the row was not a dedupe hit. That guard is load-bearing: hydration stamps `_thread_id` onto every message it writes, so without it each hydrated message would re-hydrate its own thread (measured 10× amplification, comments at extraction_tasks.py:606-628, where `thread_ext_id` is computed).
...
`extraction.correlate_evidence` schedules `extraction.reconstruct_episode` with a **180-second debounce** when it created correlations. `_reconstruct` (extraction_tasks.py:1008) then runs these gates in order, each of which exists to avoid paying for narration a later sweep would retire:
...
9. **AE Ops Context Graph alignment** (migration `0029`)
   ...
   **Status update — "no service code changes, population is the next wave" is no longer true, and the difference matters when auditing this schema.** A dedicated audit found **79** columns across the codebase declared by a model and written by nothing. Several from this group have since gained writers: the execution step's `action_name` / `action_type` / `execution_mode` / `executed_by`, the approval's `action_name` / `approver_role`, and the decision's `decision_intent` / `risk_level`. `idempotency_key` and `duplicate_check_status` are now live too — the key is derived from an approved artifact's hash scoped to the case, assigned only to side-effecting steps, and a duplicate is skipped-and-recorded rather than replayed.
   ...
   One dormant chain to be explicit about: **nothing constructs `FixPattern`**. It is read in six modules (`fix_cohort_service`, `fix_applicability_service`, `execution_verification_service`, `case_outcome_service`, `graph/agent/materializer.py`, `graph/agent/hydrators.py`), so the fix-applicability join, the cohort counters, and the verification fix-outcome write-back are dormant rather than merely unexercised — nothing can mint the row the whole chain keys on.
...
- **The parent evidence embedding is not budget-gated or cost-attributed.** Its call site passes no `tenant_id`/`db` (backend/src/contextedge/workers/extraction_tasks.py:65-70), unlike the chunk batch path, which passes both (backend/src/contextedge/workers/chunk_tasks.py:165-171).
...
**Citation grounding pass, 2026-08-19.** Every `file:line` above was re-opened and checked against the working tree. The corrections made: the `extraction_tasks.py` normalization citations had drifted by roughly thirteen lines (relevance fail-open, error signatures, identity, decisions, parent embedding, the post-commit fan-out and the hydration-amplification comment), `_reconstruct` sits at 1008 rather than 995, `extraction.normalize_evidence` at 1319 rather than 1304, and the chunk path's budget gate and cost attribution live at `chunk_tasks.py:165-171` — not in the task wrapper.
