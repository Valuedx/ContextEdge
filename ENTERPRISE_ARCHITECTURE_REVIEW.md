...
### Weeks 13–14: retrieval quality (added post-review)

A retrieval-completeness gap surfaced after the original review: `embed_evidence(title, body[:8000])` made any body content past ~8 KB invisible to semantic retrieval, even though the full body landed in Postgres. Long Teams threads, post-mortems, and log attachments all suffered. The fix is the per-source chunking pipeline.

19. **Shipped 2026-05-08** — Evidence chunking foundation. Migration `0030_evidence_chunks` adds the `evidence_chunks` sibling table (FK `evidence_id → evidence_items(id) ON DELETE CASCADE`) with one row per per-source-meaningful unit (Jira description / ServiceNow short description, Teams message, Gmail reply, runbook heading section, log event). HNSW index on the chunk embedding column with the same `m = 16, ef_construction = 64` parameters as `0021`; GIN `jsonb_path_ops` on `metadata` matching `0025`; partial B-tree `(tenant_id, ingested_at DESC) WHERE chunked_at IS NULL` driving the future backfill. New `services/chunkers/` package implements pure-function `Chunker` Protocol with lazy registry mirroring `connectors/registry.py`. Concrete chunkers: `TicketChunker` (Jira / ServiceNow metadata enrichment — priority, status, issue_type, project, author), `ThreadChunker` (Gmail quote-stripping with three quote-leader patterns + `>`-line detection, plus author / ts metadata), `AttachmentChunker` (markdown heading split with breadcrumb, JSONL log windowing, plain-log timestamp boundaries, prose fallback), `FallbackChunker` (recursive paragraph → line → sentence → hard split with overlap). `services/evidence_chunk_service.write_chunks` persists with chunk-level content hashing, `chunker_version` for re-chunk safety, and connector-defaulted `source_authority` tagging. Two new Celery tasks (`workers/chunk_tasks.py`) on the existing `extraction` queue: `chunk_evidence_task` (async path for large items, idempotent on `chunker_version`) and `embed_chunks_batch_task` (32-chunk batches via `generate_embeddings_batch`, respects the per-tenant LLM budget gate from `0023`). `_normalize` wired via `_dispatch_chunking`: inline for ticket / thread bodies under 16 KB on the allowlist (`jira_sm`, `servicenow`, `gmail`, `teams`); async for everything else. Wrapped in `try/except` so a chunker bug cannot regress today's parent-embedding retrieval. Closes the 4 KB / 8 KB / 2 KB extraction and embedding cliffs as a *retrieval* concern (the per-extractor caps remain in effect at extraction time but the content stays recoverable). 26 unit tests covering offset accuracy, paragraph / sentence / hard-split fallback, overlap, all three Gmail quote patterns, markdown breadcrumb composition, JSONL log windowing, plain-log timestamp boundaries, registry resolution. Detail in [codewiki/CHUNKING_DESIGN.md](codewiki/CHUNKING_DESIGN.md).

   **Still deferred:** search-side rollup (`vector_search.py` still queries `evidence_items.embedding` only — the chunk-level rollup with hybrid retrieval + MMR + parent grouping is the next ship); backfill task for legacy `chunked_at IS NULL` rows; tree-sitter code chunker; per-tenant authority-override table; per-chunk decision / identity extraction (closes the 4 KB extractor cap as an *extraction* concern, not just retrieval). Tracked in [codewiki/KNOWN_GAPS.md](codewiki/KNOWN_GAPS.md) "Resolved: Evidence chunking foundation".
...
- ~~**Long-body retrieval cliff**~~ — **mitigated 2026-05-08** via the chunking foundation in §6 item 19. The original review missed this because the symptom (recall on long bodies) was observable only at retrieval time, not in the cost / scale picture this review focused on. Adding it here so future reviews start with a complete picture.
...
| Long-body retrieval cliff (resolved): `embed_evidence(title, body[:8000])` truncated past 8 KB | `ai/embeddings.py:31`. **Mitigated 2026-05-08** by `0030_evidence_chunks` + `services/chunkers/` (§6 item 19). |
