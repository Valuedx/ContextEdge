...
async def transition_playbook(
    ...
    *,
    ...
    if new_state == "approved" and approved_version is not None:
        ...
        # Always (re)embed the version being published. Skipping when
        # embedding is already set left fingerprints pointing at v1.0.0
        # (or at an unpublished draft written at version-create) forever.
